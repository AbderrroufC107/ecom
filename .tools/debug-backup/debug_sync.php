...
if (!empty($request['success']) && !empty($request['json'])) {
    ...
    echo "\nParsed Data Items:\n";
    print_r(array_keys($data_items));

    foreach ($data_items as $tracking_number => $data) {
        if (!isset($orders_map[$tracking_number])) {
            echo "Tracking $tracking_number not in map!\n";
            continue;
        }
        ...
        echo "Updating $tracking_number to status: $latest_status\n";
    ...
    echo "Request success was false or json empty.\n";
}

echo "\nSynced $synced_count orders.\n";

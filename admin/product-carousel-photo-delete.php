...
$statement = $dbRepo->prepare("SELECT photo, p_id FROM tbl_product_photo WHERE pp_id = ? AND is_additional = 1");
...
$statement = $dbRepo->prepare("DELETE FROM tbl_product_photo WHERE pp_id = ? AND is_additional = 1");
$statement->execute([$photo_id]);

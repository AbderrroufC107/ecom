...
$statement = $dbRepo->prepare("SELECT mcat_id FROM tbl_mid_category WHERE mcat_id=?");
...
$statement = $dbRepo->prepare("SELECT ecat_id FROM tbl_end_category WHERE mcat_id=?");
$statement->execute([$mid_id]);
...
$dbRepo->prepare("DELETE FROM tbl_mid_category WHERE mcat_id=?")->execute([$mid_id]);

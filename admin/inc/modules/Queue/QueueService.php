...
class QueueService
{
    ...
    {
        $stmt = $pdo->prepare("INSERT INTO tbl_queue_jobs (store_id, type, payload, priority, scheduled_at)
            VALUES (?, ?, ?, ?, COALESCE(?, NOW()))");
    ...
    {
        $stmt = $pdo->prepare("SELECT * FROM tbl_queue_jobs
        ...
        $update = $pdo->prepare("UPDATE tbl_queue_jobs SET status = 'processing', started_at = NOW(),
        ...
        ?string $errorMessage = null): void
    {
        ...
        $params[] = $jobId;
        $stmt = $pdo->prepare("UPDATE tbl_queue_jobs SET " . implode(', ', $sets) . " WHERE id = ?");
        $stmt->execute($params);
    }
    ...
    public static function getStats(PDO $pdo): array
    {
        ...
        try {
            $result = $pdo->query("SELECT status, COUNT(*) AS cnt FROM tbl_queue_jobs GROUP BY status");
        ...
        ?string $statusFilter = null, ?string $typeFilter = null): array
    {
        ...
        $stmt = $pdo->prepare("SELECT * FROM tbl_queue_jobs WHERE {$where}
    ...
    public static function getJobCount(PDO $pdo, ?string $statusFilter = null, ?string $typeFilter = null): int
    {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM tbl_queue_jobs WHERE {$where}");
        $stmt->execute($params);
    ...
    {
        $stmt = $pdo->prepare("SELECT * FROM tbl_queue_jobs WHERE id = ?");
        $stmt->execute([$jobId]);
        $row = $stmt->fetch();
        return $row ?: null;
    }
    ...
    {
        $stmt = $pdo->prepare("UPDATE tbl_queue_jobs SET status = 'pending', error_message = NULL,
    ...
    {
        $stmt = $pdo->prepare("UPDATE tbl_queue_jobs SET status = 'cancelled' WHERE id = ?");
        $stmt->execute([$jobId]);
    }
    ...
    {
        $offset = ($page - 1) * $perPage;
        $stmt = $pdo->prepare("SELECT * FROM tbl_failed_jobs ORDER BY id DESC LIMIT ? OFFSET ?");
    ...
    {
        $stmt = $pdo->query("SELECT COUNT(*) FROM tbl_failed_jobs");
        return (int) $stmt->fetchColumn();
    }
    ...
    {
        $stmt = $pdo->prepare("INSERT INTO tbl_failed_jobs (original_job_id, type, payload, priority,
    ...
    {
        $stmt = $pdo->prepare("DELETE FROM tbl_failed_jobs WHERE id = ?");
        $stmt->execute([$failedJobId]);
    }

    public static function requeueFailedJobs(PDO $pdo): int
    {
        $failed = $pdo->query("SELECT * FROM tbl_failed_jobs")->fetchAll();
        $count = 0;
        foreach ($failed as $job) {
            ...
            $del = $pdo->prepare("DELETE FROM tbl_failed_jobs WHERE id = ?");
            $del->execute([$job['id']]);
            $count++;
        }
        return $count;
    ...
    {
        $stmt = $pdo->prepare("DELETE FROM tbl_queue_jobs WHERE status = 'completed'
    ...
    {
        $stmt = $pdo->prepare("DELETE FROM tbl_failed_jobs WHERE failed_at < DATE_SUB(NOW(), INTERVAL ? DAY)");
        $stmt->execute([$olderThanDays]);
        return $stmt->rowCount();
    }

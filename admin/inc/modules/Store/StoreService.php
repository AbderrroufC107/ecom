...
class StoreService
{
    ...
    {
        $stmt = $pdo->prepare("SELECT * FROM tbl_store_users WHERE email = ? AND status = 'active'");
        $stmt->execute([$email]);
        $user = $stmt->fetch();
        ...
        string $password, string $role = 'staff'): int
    {
        ...
        $stmt = $pdo->prepare("INSERT INTO tbl_store_users (store_id, name, email, password, role)
            VALUES (?, ?, ?, ?, ?)");
    ...
    {
        $stmt = $pdo->prepare("SELECT * FROM tbl_store_users WHERE store_id = ? ORDER BY id ASC");
    ...
    {
        $stmt = $pdo->prepare("SELECT setting_value FROM tbl_store_settings WHERE store_id = ? AND setting_key = ?");
    ...
    {
        $stmt = $pdo->prepare("INSERT INTO tbl_store_settings (store_id, setting_key, setting_value)
            VALUES (?, ?, ?)
    ...
    {
        $stmt = $pdo->prepare("SELECT setting_key, setting_value FROM tbl_store_settings WHERE store_id = ?");
        $stmt->execute([$storeId]);
        $result = [];
    ...
    public static function getStats(PDO $pdo, int $storeId): array
    {
        ...
        try {
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM tbl_store_users WHERE store_id = ?");
            $stmt->execute([$storeId]);
        ...
        try {
            $stmt = $pdo->prepare("SELECT COALESCE(SUM(total), 0) FROM tbl_invoices WHERE store_id = ? AND status = 'paid'");
            $stmt->execute([$storeId]);
    ...
    public static function getGlobalStats(PDO $pdo): array
    {
        ...
        try {
            $total = (int) $pdo->query("SELECT COUNT(*) FROM tbl_stores")->fetchColumn();
            $active = (int) $pdo->query("SELECT COUNT(*) FROM tbl_stores WHERE status = 'active'")->fetchColumn();
            $trial = (int) $pdo->query("SELECT COUNT(*) FROM tbl_stores WHERE status = 'trial'")->fetchColumn();
            $suspended = (int) $pdo->query("SELECT COUNT(*) FROM tbl_stores WHERE status = 'suspended'")->fetchColumn();
            $revenue = (float) $pdo->query("SELECT COALESCE(SUM(total), 0) FROM tbl_invoices WHERE status = 'paid'")->fetchColumn();
        } catch (\PDOException $e) {
        }
    ...
    {
        $stmt = $pdo->prepare("SELECT * FROM tbl_store_themes WHERE store_id = ?");
        $stmt->execute([$storeId]);
        $row = $stmt->fetch();
        return $row ?: null;
    }
    ...
    public static function saveTheme(PDO $pdo, int $storeId, array $themeData): void
    {
        ...
        $stmt = $pdo->prepare("INSERT INTO tbl_store_themes (store_id, theme_json)
            VALUES (?, ?)

...
class PaymentService
{
    ...
    { global $dbRepo;
        $stmt = $dbRepo->prepare("INSERT INTO tbl_payments (invoice_id, store_id, amount, method, transaction_id, status)
            VALUES (?, ?, ?, ?, ?, 'completed')");
        $stmt->execute([$invoiceId, $storeId, $amount, $method, $transactionId]);
        $paymentId = (int) $dbRepo->lastInsertId();
    ...
    { global $dbRepo;
        $stmt = $dbRepo->prepare("SELECT * FROM tbl_payments WHERE invoice_id = ? ORDER BY id ASC");
    ...
    { global $dbRepo;
        ...
        $stmt = $dbRepo->prepare("SELECT p.*, i.invoice_number
            FROM tbl_payments p
    ...
    { global $dbRepo;
        $stmt = $dbRepo->prepare("SELECT * FROM tbl_payments WHERE transaction_id = ?");
    ...
    { global $dbRepo;
        if ($storeId) {
            $stmt = $dbRepo->prepare("SELECT COALESCE(SUM(amount), 0) FROM tbl_payments WHERE store_id = ? AND status = 'completed'");
            $stmt->execute([$storeId]);
        } else {
            $stmt = $dbRepo->query("SELECT COALESCE(SUM(amount), 0) FROM tbl_payments WHERE status = 'completed'");
        }
    ...
    { global $dbRepo;
        $stmt = $dbRepo->prepare("SELECT COALESCE(SUM(amount), 0) FROM tbl_payments

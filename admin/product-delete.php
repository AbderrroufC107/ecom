...
<?php
	...
	$statement = $dbRepo->prepare("DELETE FROM tbl_product_photo WHERE p_id=?");
	$statement->execute([$id]);

	// Delete from tbl_product_size
	$statement = $dbRepo->prepare("DELETE FROM tbl_product_size WHERE p_id=?");
	$statement->execute([$id]);

	// Delete from tbl_product_color
	$statement = $dbRepo->prepare("DELETE FROM tbl_product_color WHERE p_id=?");
	$statement->execute([$id]);

	// Delete from tbl_rating
	$statement = $dbRepo->prepare("DELETE FROM tbl_rating WHERE p_id=?");
	$statement->execute([$id]);

	// Delete from tbl_order
	$statement = $dbRepo->prepare("DELETE FROM tbl_order WHERE product_id=?");
	$statement->execute([$id]);

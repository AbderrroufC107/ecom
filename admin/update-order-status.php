...
if(isset($_POST['order_id']) && isset($_POST['status'])) {
    ...
    $order_lookup = $dbRepo->prepare("SELECT id, order_id, order_status, customer_name, customer_phone, wilaya, commune, address, delivery_type, customer_ip, device_id FROM tbl_order WHERE order_id=? LIMIT 1");
    $order_lookup->execute([$_POST['order_id']]);
    $order = $order_lookup->fetch(PDO::FETCH_ASSOC) ?: [];
    $statement = $dbRepo->prepare("UPDATE tbl_order SET order_status=? WHERE order_id=?");

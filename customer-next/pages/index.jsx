...
import { getCatalog, phpBase } from "../lib/api";
...
function OrderSummary({ order, children }) {
  return (
    <article className="serviceOrderCard">
      <div>
        <span className="serviceOrderId">#{order.publicId || order.id}</span>
        <h3>{order.productName || "منتج غير محدد"}</h3>
        <p>
          الكمية: {order.quantity || 0} | المجموع: {money(order.totalPrice)}
        </p>
        <div className="serviceOrderMeta">
          <span>{order.status || "قيد المعالجة"}</span>
          {order.remoteStatus ? <span>التوصيل: {order.remoteStatus}</span> : null}
          {order.tracking ? <span>تتبع: {order.tracking}</span> : null}
        </div>
      </div>
      {children}
    </article>
